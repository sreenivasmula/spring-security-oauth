...
public class TokenRevocationLiveTest {
    ...
    private String obtainAccessToken(String clientId, String username, String password) {
        ...
        final Response response = RestAssured.given().auth().preemptive().basic(clientId, "secret").and().with().params(params).when().post("http://localhost:8080/spring-security-oauth-server/oauth/token");
    ...
    private void authorizeClient(String clientId) {
        ...
        final Response response = RestAssured.given().auth().preemptive().basic(clientId, "secret").and().with().params(params).when().post("http://localhost:8080/spring-security-oauth-server/oauth/authorize");
    }
